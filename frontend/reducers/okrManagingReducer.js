...
function softDeleteKeyResult(oldObjectives, keyResultId, objectiveId, flag) {
	...
	if(objectiveIndex !== -1) {
		...
		let defaultKeyResultIndex = objectives[objectiveIndex].defaultKeyResults.findIndex((keyResult) => {
			return keyResult === keyResultId;
		});

		if(defaultKeyResultIndex !== -1) {
			objectives[objectiveIndex].defaultKeyResults.splice(defaultKeyResultIndex, 1);
		}
	}
